...
- Cite run names, field names, and row numbers when they appear in the pack.
...
def answer(db: Session, user: User, payload: ChatRequest) -> ChatResponse:
    ...
    try:
        reply = bedrock_llm.chat(SYSTEM_PROMPT, user_prompt, max_tokens=800)
    except Exception as exc:
        raise ValueError(f"Chat model failed: {exc}") from exc
    return ChatResponse(reply=(reply or "").strip() or REFUSE_MESSAGE, refused=False, page=page)
...
def build_context_pack(db: Session, user: User, ctx: ChatContextIn) -> dict:
    pack: dict = {
        ...
        "projects": _pack_projects(db, user),
    }
...
def _pack_projects(db: Session, user: User) -> list[dict]:
    return [{"id": str(p.id), "name": p.name} for p in _owned_projects(db, user)]
