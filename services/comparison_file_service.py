...
Comparison files can reach 200k rows, so reads use calamine (openpyxl
read_only fallback) and the annotated report is streamed out with
...
def read_header(file_bytes: bytes) -> list[str]:
    try:
        ...
        frame = pl.read_excel(
            io.BytesIO(file_bytes), engine="calamine", infer_schema_length=0, n_rows=1,
        )
        return [str(name).strip() if name is not None else "" for name in frame.columns]
    except Exception:
...
def iter_data_rows(file_bytes: bytes) -> Iterator[tuple[int, tuple]]:
    ...
    try:
        yield from _iter_xlsx_calamine(file_bytes)
        return
    except Exception:
        logger.exception("calamine XLSX read failed; falling back to openpyxl read_only")
    yield from _iter_xlsx_openpyxl(file_bytes)


def _emit_rows(rows) -> Iterator[tuple[int, tuple]]:
    excel_row = 1
    emitted = 0
    for raw in rows:
        excel_row += 1
        ...
        yield excel_row, tuple("" if cell is None else cell for cell in raw)


def _iter_xlsx_calamine(file_bytes: bytes) -> Iterator[tuple[int, tuple]]:
    import polars as pl

    frame = pl.read_excel(io.BytesIO(file_bytes), engine="calamine", infer_schema_length=0)
    yield from _emit_rows(frame.iter_rows())


def _iter_xlsx_openpyxl(file_bytes: bytes) -> Iterator[tuple[int, tuple]]:
    wb = load_workbook(io.BytesIO(file_bytes), read_only=True, data_only=True)
    ws = wb.active
    try:
        rows = ws.iter_rows(values_only=True)
        next(rows, None)  # header
        yield from _emit_rows(rows)
    finally:
        wb.close()
...
class AnnotatedResultWriter:
    ...
    def _write_cell(self, row: int, col: int, value, red: bool) -> None:
        ...
        if isinstance(value, (datetime, date, time)):
            ...
            return
        self._sheet.write_string(row, col, str(value), cell_format)
